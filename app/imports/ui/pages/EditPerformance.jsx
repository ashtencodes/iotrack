...
const bridge = new SimpleSchema2Bridge(Performances.schema);
...
class EditPerformance extends React.Component {
  ...
  submit(data) {
    const { athleteID, eventID, raceType, raceTime, throwDistance, jumpDistance, jumpHeight, ranking, _id } = data;
    Performances.collection.update(_id, { $set: { athleteID, eventID, raceType, raceTime, throwDistance, jumpDistance, jumpHeight, ranking } }, (error) => (error ?
  ...
  renderPage() {
    return (
      <Grid container centered>
        <Grid.Column>
          <Header as="h2" textAlign="center">Edit Performance</Header>
...
export default withTracker(({ match }) => {
  ...
  // Get the document
  const doc = Performances.collection.findOne(documentId);
  return {
    doc,
    ready,
  };
